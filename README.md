...
- [ ] **多尺度训练**：支持 2×/4× 同一流程，或 2× 专用 backbone/head，便于直接服务 2× lowres。  
- [ ] **数据增强**：在现有退化基础上增加轻度随机（如微小缩放、亮度/对比度），提升泛化。  
- [ ] **无参考指标**：系统化记录 NIQE/MANIQA/CLIPIQA，并做多次实验的统计与可视化。  
- [ ] **轻量化与部署**：探索模型剪枝、量化或导出 ONNX，便于在本地/边缘部署。  
- [ ] **真实 LR 测试**：在 Diffusion 实际生成的壁纸粗图上测试，验证退化假设与真实场景的一致性。
